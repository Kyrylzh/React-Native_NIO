...
class EditSubTask extends Component {
  ...
  async getUserDetails() {
    ...
    try {
      ...
      let subTasktaskDeatails = await AsyncStorage.getItem("subTaskDetails");

      console.log("In Create Task Admin  : " + userDeatails + " ********** ");
      console.log("In Create Task Admin  : " + taskDeatails + " ********** ");
      console.log(
        "In Create Task Admin  : " + subTasktaskDeatails + " ********** "
      );
      ...
      this.setState({
        ...
        subTaskDetails: subTaskDetailsJson,
        subTask: subTaskDetailsJson.subtask,
        startDate: subTaskDetailsJson.start_date,
        finishedDate: subTaskDetailsJson.end_date
      });
      console.log(
        ...
        "  Task subTaskDetails in state data  ",
        this.state.subTaskDetails
      );

      // this.setSubTaskId(this.state.subTaskDetails.subtask);
    } catch (error) {
      alert(error);
    }
  }
  ...
  setSubTaskId(data) {
    console.log("setSubTaskId data ", data);
    this.setState({
      text: data
    });

    console.log(
      "In setSubTaskId state details   ********** " + this.state.text
    );
  }

  getAllUsers = async () => {
    console.log("Fetching.... getAllUsers in Create Task From Admin");
    await fetch("http://cupdes.com/api/v1/get-allusers/1", {
      method: "GET",
      headers: {
  ...
  handleNextButton = () => {
    ...
    this.requestUpdateSubTask();
    // this.props.navigation.navigate("CreateSubTaskUses");
  };

  async requestUpdateSubTask() {
    const task_id = this.state.taskDetails.id;
    const subTask_id = this.state.subTaskDetails.id;
    ...
    var data = {
      ...
      //   "user_id":2,
      // "subtask_id":2,
      // "subtask":"hello nio mini phase 1",
      // "start_date":"2019-06-27",
      // "End_date":""
    };

    console.log(" Update sub task details ", JSON.stringify(data));

    try {
      let response = await fetch("http://cupdes.com/api/v1/update-subtask", {
        method: "POST",
        headers: {
          "X-AUTH-TOKEN": "Px7zgU79PYR9ULEIrEetsb",
        ...
        .then(responseJson => {
          ...
          this.updateSubTaskDataHandler(responseJson);
        });
    } catch (errors) {
      alert(errors);
    }
  ...
  updateSubTaskDataHandler = data => {
    console.log("In data Handler update SubTaskDataHandler : ", data);
    // this.setSubTaskId(data.data.id);
    // this.props.navigation.navigate("TaskSLAPAdminList");Screen2
