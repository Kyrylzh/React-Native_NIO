...
class AddDiscussionUser extends Component {
  constructor(props) {
    super(props);
    this.state = {
      text: "",
  ...
  async requestCreateDiscussion() {
    ...
    if (subTask_userId.length == 0) {
      ...
      var data = {
        ...
        question_id: this.state.discussionId,
        responder_ids: this.state.subTask_userId
      };

      console.log(" in data requestCreateDiscussion ", JSON.stringify(data));

      try {
        let response = await fetch(
          "http://cupdes.com/api/v1/create-questionusers",
          {
            method: "POST",
            headers: {
              "X-AUTH-TOKEN": "Px7zgU79PYR9ULEIrEetsb",
...
export default AddDiscussionUser;
